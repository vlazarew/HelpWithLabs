...
CREATE TABLE IF NOT EXISTS `communal_services`.`consumer` (
  ...
  CONSTRAINT `fk_consumer_credentials1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_consumer_phone_number1`
    FOREIGN KEY (`phone_number_id`)
    REFERENCES `communal_services`.`phone_number` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE INDEX `fk_consumer_phone_number1_idx` ON `communal_services`.`consumer` (`phone_number_id` ASC) VISIBLE;
...
CREATE UNIQUE INDEX `id_UNIQUE` ON `communal_services`.`phone_number` (`id` ASC) VISIBLE;
